...
![吵架练习室首页场景选择界面](docs/images/lobby.jpg)
...
9. 沉浸模式每轮结束后，裁判只读取当前会话和新增轮次，判断是否达到表达目标，并持久化判决。
10. 表达目标达成后，页面展示沟通成果和即时心理状态文案，再由用户决定是否生成复盘报告。
...
- 对话流式返回
- 多用户匿名会话隔离
- SQLite 持久化会话、完整消息、轮次状态、复盘和生成任务
- 表达目标达成后可保存双方文字与语音，并通过独立回放链接分享
- 同一会话内请求加锁，避免并发写入错乱
- `turns` 状态模型跟踪 pending、streaming、completed、failed，失败重试不会留下半轮消息
- 本地缓存用户创建过的场景，首页最多展示 3 个最新记录
- Enter 发送消息，Shift+Enter 换行
- 每个场景支持训练模式和沉浸语音模式
- 沉浸模式首句自动朗读，支持一轮一轮录音、识别、回复和播放
- 沉浸模式逐轮裁判，表达目标达成后展示成果与即时心理状态，并提供复盘入口
